...
const SCRAPE_ERROR = (err, url = undefined) => {
  const msg = url
    ...
    : `Failed to scrape the page at ${url}`;
  console.warn(msg);
  console.error(err);
  return { error: msg, reason: err };
};
...
/**
 * @typedef {{uploaded_by?:string, thumbnail?: string, href: string, title: string, durationS: number}} PlaylistInfo
 */

/**
 * @note may download youtube recommended sometimes... trying to fix
 * @param {string} playlistUrl the url containing the 'list=' query parameter pointing to the youtube playlist
 * @returns {Promise<PlaylistInfo[]>} the info for all videos in the playlist
 */
async function getPlaylistInfo(playlistUrl) {
  ...
  for (let i = 0; i < results.length; i++) {
    ...
    results[i]["uploaded_by"] = resolved[i].videoDetails.author.name;
    results[i]["thumbnail"] = resolved[i].videoDetails.thumbnails[0].url;
    results[i]["durationS"] = resolved[i].videoDetails.lengthSeconds;
    results[i]["TRCK"] = i + 1;
  }
